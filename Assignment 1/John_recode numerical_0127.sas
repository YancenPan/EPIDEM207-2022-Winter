...
data epi207.newjohn2;
	...
	death_cause=input(substr(F13,1,1),1.);
	audit_score=input(substr(F14,1,1),1.);
	aud_absrisk=input(compress(substr(F15,1,2), "."),2.);
	audrisk_absrisk=input(compress(substr(F16,1,2), "."),2.);
	smoke_stat=input(substr(F18,1,1),1.);
	alab_risk=input(compress(substr(F17,1,2), "."),2.);
run;
proc contents data=epi207.newjohn2;
run;

/*add varialbe label*/
data epi207.newjohn;
	set epi207.newjohn2 (drop=F1-F19);
