/*************************************************************
This code was generated by Yancen Pan & Melissa Soohoo, revised by Yancen Pan
...
/*Note: 
The file HW.john_clean.sas7bdat was derived in prior codes and shared with YP for analyses. 
MS performed Table 1 & 3 
YP performed Table 2 & Figure 1 
...
proc phreg data =hw2.john_clean2  ;
	...
	format smoke;
title "Table 2a model 1: smoke  &out"; 
run;  
...
proc phreg data =hw2.john_clean2  ;
	...
	format smoke; 
title "Table 2b model 2: smoke &out adj age and sex"; 
run;  
...
proc phreg data =hw2.john_clean2  ;
	...
	format smoke;
title "Table 2c model 3: smoke &out adj age, sex, health, edu, and alcohol consumption "; 
run;  

/*print out with 2 decimals*/
proc print data=table2a;
	title "table 2a model 1 print";
	format hazardratio 8.2 HRLowerCL 8.2 HRUpperCL 8.2;
run;

proc print data=table2b;
	title "table 2b model 2 print";
	format hazardratio 8.2 HRLowerCL 8.2 HRUpperCL 8.2;
run;

proc print data=table2c;
		title "table 2c model 3 print";
	format hazardratio 8.2 HRLowerCL 8.2 HRUpperCL 8.2;
run;
...
**************************************************************
*               Make Figure 1 and Figure 2					 *
*				Total Mortality								 *
**************************************************************


/*Figure 1 was the age/sex adjusted survival function.
Calculated the reference values based on the whole cohort. 
*/;

Proc means data = hw2.john_clean2; 
var age ;
run; 
data covs;
input smoke female age health edu alc_cons; 
datalines; 
0	1	41.7174777	3	3	6
1	1	41.7174777	3	3	6
2	1	41.7174777	3	3	6
3	1	41.7174777	3	3	6
4	1	41.7174777	3	3	6
;
run; 
ods graphics on; 
ods exclude Modelinfo classlevelinfo censoredsummary 
ConvergenceStatus FitStatistics GlobalTests ModelANOVA;
...
proc phreg data = hw2.john_clean2 plots(overlay)=survival ;
	...
	format smoke health edu alc_cons ; 
	baseline covariates=covs out=base/rowid= smoke;
run; 

/*create formats for the graph*/;
options fmtsearch= (hw);
proc format library=hw;
value	test		0				=	"0:never smoker"
					1				=	"1:ever less than daily"
					2				=	"2:former daily"
					3				=	"3:current daily <20 cpd"
					4				=	"4:current daily >=20 cpd"
					;
run; 

data _surv2;
set _surv1; 
smoke2 = input(smoke, best12.);
format smoke2 test.;
run; 
title " " ;
/*Figure 1*/;
...
step x=time y=survival/group = smoke2; 
...
/*Figure 2 was the age/sex/education/health status/alcohol consumption adjusted survival function.
Calculated the reference values based on the whole cohort. 
*/;
ods graphics on; 
